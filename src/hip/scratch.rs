...
//!
//! This module provides `PrefillScratch` for reusable intermediate buffers and
...
/// Pre-allocated scratch buffers for HIP prefill (FLA) inference.
///
/// Contains all intermediate buffers needed for the FLA chunked prefill
/// pipeline, including standard computation buffers, LoRA buffers, FLA-specific
/// buffers, and persistent GPU state. This struct is used by both `Rwkv7Hip`
/// (for backward compatibility) and `HipPrefill` (standalone prefill module).
///
...
/// calls, eliminating allocation overhead.
///
/// # Buffer Categories
///
...
/// 4. **FLA buffers** (f32): Chunked prefill intermediates
/// 5. **Output buffer** `[n_vocab, T, B]`: Final logits
///
/// # Usage
///
/// ```rust,ignore
...
pub type HipScratch = PrefillScratch;
...
/// Configuration for the standalone prefill module ([`HipPrefill`]).
///
/// Controls buffer sizing and batching for FLA chunked prefill inference.
/// This wraps the same parameters as [`HipRuntimeConfig`] in a purpose-specific type.
#[derive(Debug, Clone)]
...
impl PrefillConfig {
    /// Create a new prefill config with specified chunk size and batch size.
    pub fn new(max_prefill_chunk: usize, batch_size: usize) -> Self {
        Self {
            max_prefill_chunk,
            batch_size,
    ...
    /// Convert to a `HipRuntimeConfig` for scratch buffer allocation.
    pub fn to_runtime_config(&self) -> HipRuntimeConfig {
        HipRuntimeConfig {
...
/// Configuration for the standalone decode module.
///
...
impl DecodeConfig {
    /// Create a new decode config with specified batch size.
...
/// # Buffer Categories
///
/// 1. **Standard buffers** `[n_embd, 1, B]`: Main computation intermediates
/// 2. **FFN buffers** `[n_hidden, 1, B]`: Feed-forward network intermediates
/// 3. **LoRA buffers** `[lora_dim, 1, B]`: Low-rank adaptation intermediates
/// 4. **Output buffer** `[n_vocab, 1, B]`: Final logits
///
/// # Memory
///
/// With T=1 and no FLA buffers, `DecodeScratch` uses approximately 12 MB
/// versus ~95 MB for `PrefillScratch`.
#[derive(Debug)]
pub struct DecodeScratch {
    ...
    /// Persistent GPU state (resident on device)
    pub att_shift_state_gpu: Vec<TensorHip<f16>>,
    pub ffn_state_gpu: Vec<TensorHip<f16>>,
    pub wkv_state_gpu: Vec<TensorHip<f32>>,

    // ========== Standard buffers [n_embd, 1, B] ==========
    /// Main hidden state (persists across layers within forward pass)
    pub x: TensorHip<f16>,
    ...
    /// Attention output (combined, gated, projected)
    pub att_out: TensorHip<f16>,
    ...
    // ========== FFN hidden buffers [n_hidden, 1, B] ==========
    /// FFN key projection output
    pub ffn_k: TensorHip<f16>,
    ...
    /// Pinned host buffer for async logits download
    pub logits_staging: PinnedBuffer<f32>,

    // ========== Token staging buffer [1, B] ==========
    /// GPU staging buffer for tokens
    pub token_staging: TensorHip<u32>,

    /// GPU buffer for sequence lengths (masked kernels)
    pub lens_gpu: TensorHip<i32>,

    // ========== Pinned host staging buffer [n_embd * 1 * B] ==========
    /// Pinned host buffer for async embedding upload
    pub emb_staging: PinnedBuffer<f16>,
}

impl DecodeScratch {
    /// Create new decode scratch buffers sized for the given model and config.
    ///
    /// All buffers are sized for T=1. No FLA buffers are allocated.
    ///
    /// # Arguments
    /// * `info` - Model dimensions (n_embd, n_hidden, n_vocab, etc.)
    /// * `lora_dims` - LoRA dimensions extracted from model weights
    ...
    pub fn new(
        ...
        // Standard shape [n_embd, 1, B]
        let std_shape = TensorShape::new(c, t, b, 1);

        // FFN hidden shape [n_hidden, 1, B]
        let ffn_shape = TensorShape::new(h, t, b, 1);

        // Output shape [n_vocab, 1, B]
        let out_shape = TensorShape::new(v, t, b, 1);
        let out_size = v * t * b;
