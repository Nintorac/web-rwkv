...
/// Row-owned WKV7 kernel for T=1 decode.
///
/// Uses in-place state update (single mutable state tensor, no separate in/out).
/// 1 thread per row, no cross-lane reductions, no inter-pass barriers.
pub fn wkv7_colmajor_t1(
    w_decay: &TensorHip<f16>,
    q: &TensorHip<f16>,
    k: &TensorHip<f16>,
    v: &TensorHip<f16>,
    ...
    use crate::hip::ffi::launch_wkv7_colmajor_t1;
    ...
    if t != 1 {
        return Err(HipErrorKind {
            code: -1,
            message: "wkv7_colmajor_t1 requires T=1".to_string(),
        });
    }
        ...
        || !lengths.is_contiguous()
    {
        return Err(HipErrorKind {
            code: -1,
            message: "wkv7_colmajor_t1 requires contiguous tensors".to_string(),
        });
    }

    unsafe {
        check(launch_wkv7_colmajor_t1(
            w_decay.as_ptr(),
            q.as_ptr(),
            k.as_ptr(),
            v.as_ptr(),
