...
#[cfg(feature = "hip")]
fn main() {
    use web_rwkv::hip::{self, Stream, DeviceBuffer};
    ...
    d_buf.copy_from_host(&host_data, &stream).expect("Failed to copy to device");
    
    // Copy back
    let mut result = vec![0.0f32; 1024];
    println!("Copying from device...");
    d_buf.copy_to_host(&mut result, &stream).expect("Failed to copy from device");
