...
/// Based on RWKV7_HIP_BACKEND_PLAN tolerance requirements.
#[derive(Clone, Copy)]
struct Tolerances {
    rtol: f32,
    atol: f32,
...
impl Tolerances {
    /// Standard tolerance for normalized activations (after layernorm, etc.)
    const NORMALIZED: Self = Self { rtol: 1e-3, atol: 1e-4 };

    /// Tolerance for linear projections and matrix multiplications
    const MATMUL: Self = Self { rtol: 1e-2, atol: 1e-3 };

    /// Tolerance for activations with potential numerical instability
    const ACTIVATION: Self = Self { rtol: 1e-2, atol: 1e-3 };

    /// Tolerance for WKV state (FP32 accumulation)
    const STATE: Self = Self { rtol: 1e-4, atol: 1e-5 };

    /// Looser tolerance for deeply accumulated values
    const ACCUMULATED: Self = Self { rtol: 2e-2, atol: 2e-3 };
}
