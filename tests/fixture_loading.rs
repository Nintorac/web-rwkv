...
fn test_fixture_load_sigmoid() {
    if !fixtures_exist() {
        eprintln!(
            "Skipping test: fixtures not generated. Run: python scripts/generate_test_fixtures.py"
        );
        return;
    }
    ...
    assert!(
        fixture.contains("input_shape"),
        "Missing 'input_shape' array"
    );
    assert!(fixture.contains("expected"), "Missing 'expected' array");
    assert!(
        fixture.contains("expected_shape"),
        "Missing 'expected_shape' array"
    );
...
fn test_decay_exp_fixture() {
    ...
    // Run kernel
    let output = web_rwkv::hip::hip_decay_exp(input).expect("decay_exp kernel failed");
    ...
    println!(
        "decay_exp fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_decay_exp_stability_fixture() {
    ...
    // Run kernel
    let output = web_rwkv::hip::hip_decay_exp(input).expect("decay_exp stability kernel failed");
...
fn test_lerp_fixture() {
    ...
    println!("Testing lerp: {} elements, shape {:?}", a.len(), shape);

    // Run kernel
    let output = web_rwkv::hip::hip_lerp(a, b, t).expect("lerp kernel failed");

    // Compare with fixture (< 1e-3 relative error per acceptance criteria)
    assert_tensors_close(&output, expected, 1e-3, 1e-4).expect("lerp output doesn't match fixture");
...
fn test_sigmoid_fixture() {
    ...
    // Run kernel
    let output = web_rwkv::hip::hip_sigmoid(input).expect("sigmoid kernel failed");
    ...
    println!(
        "sigmoid fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_sigmoid_edge_cases_fixture() {
    ...
    println!("Testing sigmoid edge cases: {} elements", input.len());

    // Run kernel
    let output = web_rwkv::hip::hip_sigmoid(input).expect("sigmoid edge cases kernel failed");
...
fn test_squared_relu_fixture() {
    ...
    // Run kernel
    let output = web_rwkv::hip::hip_squared_relu(input).expect("squared_relu kernel failed");
    ...
    println!(
        "squared_relu fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_softplus_decay_fixture() {
    ...
    // Run kernel
    let output = web_rwkv::hip::hip_softplus_decay(input).expect("softplus_decay kernel failed");
    ...
    println!(
        "softplus_decay fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_layer_norm_basic_fixture() {
    ...
    println!(
        "layer_norm basic fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_layer_norm_rwkv7_eps_fixture() {
    ...
    println!(
        "layer_norm rwkv7_eps fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_group_norm_fixture() {
    ...
    println!(
        "group_norm fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_l2_norm_fixture() {
    ...
    let output =
        web_rwkv::hip::hip_l2_norm(input, c, n, head_size, eps).expect("l2_norm kernel failed");

    assert_tensors_close(&output, expected, 1e-3, 1e-4)
        .expect("l2_norm output doesn't match fixture");

    println!(
        "l2_norm fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_tanh_fixture() {
    ...
    println!("Testing tanh: {} elements, shape {:?}", input.len(), shape);

    let output = web_rwkv::hip::hip_tanh(input).expect("tanh kernel failed");

    assert_tensors_close(&output, expected, 1e-3, 1e-4).expect("tanh output doesn't match fixture");
...
fn test_token_shift_single_fixture() {
    ...
    let (output, state_out) =
        web_rwkv::hip::hip_token_shift(x, state_in, mix, c, t).expect("token_shift kernel failed");
    ...
    println!(
        "token_shift single fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_wkv7_single_token_fixture() {
    ...
    let (output, state_out) =
        web_rwkv::hip::hip_wkv7(w_decay, q, k, v, a, b, state_in, n, h, t, batch)
    ...
    println!(
        "  Output: max_diff={:.2e}, gap={:.1}x",
        out_result.max_diff,
        out_result.gap_factor()
    );
    println!(
        "  State:  max_diff={:.2e}, gap={:.1}x",
        state_result.max_diff,
        state_result.gap_factor()
    );

    assert!(
        out_result.passed,
        "WKV output failed: max_diff={:.2e} ({}x)",
        out_result.max_diff,
        out_result.gap_factor() as i32
    );
    assert!(
        state_result.passed,
        "WKV state failed: max_diff={:.2e} ({}x)",
        state_result.max_diff,
        state_result.gap_factor() as i32
    );

    println!(
        "wkv7 single_token fixture test passed: {} output, {} state elements match",
        output.len(),
        state_out.len()
    );
}
...
fn test_wkv7_short_sequence_fixture() {
    ...
    println!(
        "  Output: max_diff={:.2e}, gap={:.1}x, mismatches={}/{}",
        out_result.max_diff,
        out_result.gap_factor(),
        out_result.mismatch_count,
        out_result.total_elements
    );
    println!(
        "  State:  max_diff={:.2e}, gap={:.1}x, mismatches={}/{}",
        state_result.max_diff,
        state_result.gap_factor(),
        state_result.mismatch_count,
        state_result.total_elements
    );
    ...
    println!(
        "wkv7 short_sequence fixture test passed: {} output, {} state elements match",
        output.len(),
        state_out.len()
    );
}
...
fn test_wkv7_medium_sequence_fixture() {
    ...
    println!(
        "wkv7 medium_sequence fixture test passed: {} output, {} state elements match",
        output.len(),
        state_out.len()
    );
}
...
fn test_wkv7_batched_fixture() {
    ...
    println!(
        "wkv7 batched fixture test passed: {} output, {} state elements match",
        output.len(),
        state_out.len()
    );
}
...
fn test_control_k_fixture() {
    ...
    let output = web_rwkv::hip::hip_control_k(k_a, a, k, c, t, b).expect("control_k kernel failed");
    ...
    println!(
        "control_k fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_wkv_bonus_fixture() {
    ...
    let output =
        web_rwkv::hip::hip_wkv_bonus(r, k, v, r_k, n, h, t, b).expect("wkv_bonus kernel failed");
    ...
    println!(
        "wkv_bonus fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_channel_mix_state_fixture() {
    ...
    println!(
        "channel_mix_state fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_gemv_fixture() {
    ...
    let output = web_rwkv::hip::hip_sgemm(weight, input, n, k, a).expect("SGEMM kernel failed");
    ...
    assert_tensors_close(&output, expected, 1e-3, 5e-2).expect("GEMV output doesn't match fixture");
...
fn test_gemm_batched_fixture() {
    ...
    assert!(
        result.passed,
        "GEMM output failed at spec tolerance: max_diff={:.2e} ({}x)",
        result.max_diff,
        result.gap_factor() as i32
    );
    println!(
        "GEMM batched fixture test passed: {} elements match",
        output.len()
    );
}
...
fn test_channel_mix_fixture() {
    ...
    let (after_lerp, computed_state) =
        web_rwkv::hip::hip_channel_mix_state(input, state_in, x_k, c, t, b)
            .expect("Channel mix state failed");

    println!(
        "  Steps 1-2 (token shift + lerp): {} elements",
        after_lerp.len()
    );
    ...
    let after_key_proj = web_rwkv::hip::hip_sgemm(key_weight, &after_lerp, hidden, c, t * b)
        .expect("Key projection SGEMM failed");

    println!(
        "  Step 3 (key projection): {} elements",
        after_key_proj.len()
    );
    ...
    // Step 4: Squared ReLU
    // k_sq = relu(k_proj)^2
    let after_squared_relu =
        web_rwkv::hip::hip_squared_relu(&after_key_proj).expect("Squared ReLU failed");

    println!(
        "  Step 4 (squared relu): {} elements",
        after_squared_relu.len()
    );
    ...
    let output = web_rwkv::hip::hip_sgemm(value_weight, &after_squared_relu, c, hidden, t * b)
...
fn test_full_block_fixture() {
    ...
    if !Path::new(fixture_path).exists() {
        eprintln!(
            "Skipping test: full_block fixture not generated at {}",
            fixture_path
        );
        return;
    }

    let fixture = TestFixture::load(fixture_path).expect("Failed to load full_block fixture");
    ...
    let after_ln1 = web_rwkv::hip::hip_layer_norm(input, ln1_weight, ln1_bias, c, t * b, 1e-5)
        .expect("Layer norm 1 failed");
    // Spec: Normalized values rtol=1e-3, atol=1e-4
    all_results.push(ValidationResult::check(
        "1. ln1",
        &after_ln1,
        expected_after_ln1,
        1e-3,
    ...
    let (after_token_shift, new_att_token_shift_state) =
        web_rwkv::hip::hip_channel_mix_state(&after_ln1, att_token_shift_state, x_r, c, t, b)
            .expect("Time-mix token shift failed");

    println!(
        "  Step 2a (token shift xr): {} elements",
        after_token_shift.len()
    );

    // Multiple token shifts for time-mix (xr, xw, xk, xv, xa, xg)
    let (xw_shifted, _) =
        web_rwkv::hip::hip_channel_mix_state(&after_ln1, att_token_shift_state, x_w, c, t, b)
            .expect("xw shift failed");
    let (xk_shifted, _) =
        web_rwkv::hip::hip_channel_mix_state(&after_ln1, att_token_shift_state, x_k_att, c, t, b)
            .expect("xk shift failed");
    let (xv_shifted, _) =
        web_rwkv::hip::hip_channel_mix_state(&after_ln1, att_token_shift_state, x_v, c, t, b)
            .expect("xv shift failed");
    let (xa_shifted, _) =
        web_rwkv::hip::hip_channel_mix_state(&after_ln1, att_token_shift_state, x_a, c, t, b)
            .expect("xa shift failed");
    let (xg_shifted, _) =
        web_rwkv::hip::hip_channel_mix_state(&after_ln1, att_token_shift_state, x_g, c, t, b)
            .expect("xg shift failed");
    ...
    let k_proj =
        web_rwkv::hip::hip_sgemm(k_weight, &xk_shifted, c, c, t * b).expect("K projection failed");
    let v_proj =
        web_rwkv::hip::hip_sgemm(v_weight, &xv_shifted, c, c, t * b).expect("V projection failed");
    // Spec: MatMul outputs rtol=1e-2, atol=1e-3
    all_results.push(ValidationResult::check(
        "3a. r_proj",
        &r_proj,
    ...
    let w_lora1 =
        web_rwkv::hip::hip_sgemm(w1, &xw_shifted, w_lora_dim, c, t * b).expect("W LoRA1 failed");
    // tanh activation
    let w_lora1_tanh = web_rwkv::hip::hip_tanh(&w_lora1).expect("W tanh failed");
    // @ w2 (w2 is [C, lora_dim])
    let w_lora2 =
        web_rwkv::hip::hip_sgemm(w2, &w_lora1_tanh, c, w_lora_dim, t * b).expect("W LoRA2 failed");
    // w0 + w_lora2
    let w_biased: Vec<f32> = w0
        .iter()
        .cycle()
        .take(w_lora2.len())
        .zip(w_lora2.iter())
    ...
    let a_lora1 =
        web_rwkv::hip::hip_sgemm(a1, &xa_shifted, a_lora_dim, c, t * b).expect("A LoRA1 failed");
    let a_lora2 =
        web_rwkv::hip::hip_sgemm(a2, &a_lora1, c, a_lora_dim, t * b).expect("A LoRA2 failed");
    let a_biased: Vec<f32> = a0
        .iter()
        .cycle()
        .take(a_lora2.len())
        .zip(a_lora2.iter())
    ...
    let g_lora1 =
        web_rwkv::hip::hip_sgemm(g1, &xg_shifted, g_lora_dim, c, t * b).expect("G LoRA1 failed");
    ...
    let wkv_bonus = web_rwkv::hip::hip_wkv_bonus(&r_wkv, &k_wkv, &v_wkv, r_k_weight, n, h, t, b)
        .expect("WKV bonus failed");
    let wkv_bonus_flat = reshape_nhtb_to_c(&wkv_bonus, c, t, b, n, h);

    // Combine WKV output and bonus
    let x_att_out: Vec<f32> = wkv_out_flat
        .iter()
    ...
    // ==== Step 11: Group Norm ====
    let x_att_gn =
        web_rwkv::hip::hip_group_norm(&x_att_out, ln_x_weight, ln_x_bias, c, t * b, h, 64e-5)
            .expect("Group norm failed");

    // ==== Step 12: Gate and Output Projection ====
    let x_gated: Vec<f32> = x_att_gn
        .iter()
        .zip(g_proj.iter())
        .map(|(x, g)| x * g)
        .collect();
    ...
    let after_ln2 =
        web_rwkv::hip::hip_layer_norm(&x_after_att, ln2_weight, ln2_bias, c, t * b, 1e-5)
            .expect("Layer norm 2 failed");

    // ==== Step 15: Channel-Mix (FFN) ====
    let (k_ffn, new_ffn_state) =
        web_rwkv::hip::hip_channel_mix_state(&after_ln2, ffn_state_in, x_k_ffn, c, t, b)
    ...
    let k_sq_ffn = web_rwkv::hip::hip_squared_relu(&k_proj_ffn).expect("Squared ReLU failed");
    ...
    let x_final: Vec<f32> = x_after_att
        .iter()
        .zip(x_ffn_out.iter())
        .map(|(a, b)| a + b)
        .collect();
    ...
    all_results.push(ValidationResult::check(
        "Token shift state",
        &new_att_token_shift_state,
        expected_att_token_shift_state,
        1e-3,
        1e-4,
    ));
    all_results.push(ValidationResult::check(
    ...
    println!(
        "{:<25} {:>8} {:>10} {:>10} {:>8} {:>12}",
        "Component", "Status", "max_diff", "mean_err", "gap(x)", "mismatches"
    );
    println!("{}", "-".repeat(80));
    ...
    if failures.is_empty() {
        println!(
            "All {} validations PASSED at spec tolerances!",
            all_results.len()
        );
    } else {
        println!(
            "{}/{} validations FAILED at spec tolerances:",
            failures.len(),
            all_results.len()
        );
        for f in &failures {
...
fn test_rwkv7_hip_model_loads() {
    ...
    assert_eq!(
        model.info.n_hidden, 3072,
        "Expected hidden dim 3072 (4x embd)"
    );
...
fn test_rwkv7_hip_model_dimensions() {
    ...
    assert_eq!(
        emb_len,
        65536 * 768,
        "Embedding should have n_vocab * n_embd elements"
    );
    ...
    assert_eq!(
        head_shape.dim(0),
        65536,
        "Head dim 0 should be n_vocab=65536"
    );
    ...
    assert_eq!(
        ffn_k_shape.dim(0),
        3072,
        "FFN key dim 0 should be n_hidden=3072"
    );
    assert_eq!(
        ffn_k_shape.dim(1),
        768,
        "FFN key dim 1 should be n_embd=768"
    );
...
fn test_wkv7_masked_state_preservation() {
    ...
    let mut gen_vec =
        |len: usize| -> Vec<f32> { (0..len).map(|_| rng.f32() * 2.0 - 1.0).collect() };
    ...
    println!(
        "  Short sequence: T={}, state elements={}",
        t_short,
        state_short.len()
    );
    println!(
        "  Padded sequence: T={} (real_len={}), state elements={}",
        t_padded,
        t_short,
        state_padded.len()
    );
            ...
            for hh in 0..h {
                for nn in 0..n {
                    ...
                    assert!(
                        ...
                        hh,
                        nn,
                        output_short[short_idx],
                        output_padded[padded_idx],
                        diff
                    );
                }
            }
...
fn test_wkv7_masked_batched_different_lengths() {
    ...
    let (_output, state_batched) = web_rwkv::hip::hip_wkv7_masked(
        &w_decay, &q, &k, &v, &a, &b, &state_in, &lengths, n, h, t_max, batch,
    )
...
fn debug_hip_forward_extended() {
    use web_rwkv::hip::{hip_channel_mix_state, hip_layer_norm, hip_sgemm, Rwkv7Hip, Stream};
...
fn debug_hip_vs_wgpu_divergence() {
    ...
    for t in 0..n_tokens {
        ...
        let (top_idx, top_val) = token_logits
            .iter()
            .enumerate()
    ...
    // Check state values
    println!(
        "\nAttention state[0][0:10]: {:?}",
        &state.att_states[0].as_slice()[0..10]
    );
    println!(
        "FFN state[0][0:10]: {:?}",
        &state.ffn_states[0].as_slice()[0..10]
    );
...
fn test_full_block_tight_tolerances() {
    ...
    // Step 1: Layer norm
    let x_ln1 =
        web_rwkv::hip::hip_layer_norm(&input, &ln1_weight, &ln1_bias, c, t * b, 1e-5).unwrap();

    // Step 2: Token shifts
    let (xr, _) =
        web_rwkv::hip::hip_channel_mix_state(&x_ln1, &att_token_shift_state, &x_r, c, t, b)
            .unwrap();
    let (xw, _) =
        web_rwkv::hip::hip_channel_mix_state(&x_ln1, &att_token_shift_state, &x_w, c, t, b)
            .unwrap();
    let (xk, _) =
        web_rwkv::hip::hip_channel_mix_state(&x_ln1, &att_token_shift_state, &x_k_att, c, t, b)
            .unwrap();
    let (xv, _) =
        web_rwkv::hip::hip_channel_mix_state(&x_ln1, &att_token_shift_state, &x_v, c, t, b)
            .unwrap();
    let (xa, _) =
        web_rwkv::hip::hip_channel_mix_state(&x_ln1, &att_token_shift_state, &x_a, c, t, b)
            .unwrap();
    ...
    // Step 9: WKV Bonus
    let wkv_bonus =
        web_rwkv::hip::hip_wkv_bonus(&r_wkv, &k_wkv, &v_wkv, &r_k_weight, n, h, t, b).unwrap();
    let wkv_bonus_flat = reshape_nhtb_to_c(&wkv_bonus, c, t, b, n, h);
    ...
    let wkv_diff: Vec<f32> = wkv_out_flat
        .iter()
        .zip(expected_wkv_out.iter())
        .map(|(a, b)| (a - b).abs())
        .collect();
    let wkv_max_diff = wkv_diff.iter().cloned().fold(0.0f32, f32::max);
    let wkv_mean_diff = wkv_diff.iter().sum::<f32>() / wkv_diff.len() as f32;
    println!(
        "WKV output: max_diff={:.6e}, mean_diff={:.6e} (spec atol=1e-3)",
        wkv_max_diff, wkv_mean_diff
    );

    // WKV bonus: spec says rtol=1e-2, atol=1e-3
    let bonus_diff: Vec<f32> = wkv_bonus_flat
        .iter()
        .zip(expected_wkv_bonus.iter())
        .map(|(a, b)| (a - b).abs())
        .collect();
    ...
    println!(
        "WKV bonus: max_diff={:.6e}, mean_diff={:.6e} (spec atol=1e-3)",
        bonus_max_diff, bonus_mean_diff
    );

    // WKV STATE: spec says rtol=1e-5, atol=1e-6 for FP32 state
    let state_diff: Vec<f32> = wkv_state_out
        .iter()
        .zip(expected_att_state.iter())
        .map(|(a, b)| (a - b).abs())
        .collect();
    ...
    println!(
        "WKV STATE: max_diff={:.6e}, mean_diff={:.6e} (spec atol=1e-6)",
        state_max_diff, state_mean_diff
    );

    // Find where state differs most
    let worst_idx = state_diff
        .iter()
        .enumerate()
        .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
        .map(|(i, _)| i)
...
fn test_intermediate_accuracy() {
    ...
    let worst_idx = w_diff
        ...
        .unwrap_or(0);
    println!(
        "  Worst at idx {}: HIP={:.6e}, expected={:.6e}",
    ...
    let decay_diff: Vec<f32> = hip_decay
        .iter()
        .zip(expected_decay.iter())
        .map(|(a, b)| (a - b).abs())
        .collect();
    ...
    println!(
        "  Decay: max_diff={:.6e}, mean_diff={:.6e}",
        decay_max_diff, decay_mean_diff
    );
...
fn test_wkv7_spec_tolerances() {
    ...
    // Check state
    let state_diff: Vec<f32> = state_out
        .iter()
    ...
    println!(
        "Output: max_diff={:.6e}, mean_diff={:.6e} (spec atol=1e-3)",
        out_max, out_mean
    );
    println!(
        "State:  max_diff={:.6e}, mean_diff={:.6e} (spec atol=1e-6)",
        state_max, state_mean
    );
    ...
    let worst_idx = state_diff
        ...
        .unwrap_or(0);
    println!(
        "Worst state at idx {}: HIP={:.6e}, expected={:.6e}",
    ...
    let state_rel_max = state_out
        ...
        .fold(0.0f32, f32::max);
    println!(
        "State max relative error: {:.2}% (on {} non-zero elements)",
        state_rel_max * 100.0,
        nonzero_count
    );
...
fn debug_wkv_input_comparison() {
    ...
    let wkv_b: Vec<f32> = fixture_kk
        .iter()
        .zip(fixture_a_proj.iter())
        .map(|(kk, a)| kk * a)
        .collect();
    ...
    println!(
        "  w_decay: [{:.4}, {:.4}]",
        w_wkv.iter().cloned().fold(f32::INFINITY, f32::min),
        w_wkv.iter().cloned().fold(f32::NEG_INFINITY, f32::max)
    );
    println!(
        "  r (q): [{:.4}, {:.4}]",
        r_wkv.iter().cloned().fold(f32::INFINITY, f32::min),
        r_wkv.iter().cloned().fold(f32::NEG_INFINITY, f32::max)
    );
    println!(
        "  k_ctrl: [{:.4}, {:.4}]",
        k_wkv.iter().cloned().fold(f32::INFINITY, f32::min),
        k_wkv.iter().cloned().fold(f32::NEG_INFINITY, f32::max)
    );
    println!(
        "  v: [{:.4}, {:.4}]",
        v_wkv.iter().cloned().fold(f32::INFINITY, f32::min),
        v_wkv.iter().cloned().fold(f32::NEG_INFINITY, f32::max)
    );
    println!(
        "  a: [{:.4}, {:.4}]",
        a_wkv.iter().cloned().fold(f32::INFINITY, f32::min),
        a_wkv.iter().cloned().fold(f32::NEG_INFINITY, f32::max)
    );
    println!(
        "  b: [{:.4}, {:.4}]",
        b_wkv.iter().cloned().fold(f32::INFINITY, f32::min),
    ...
    // Compare output
    let out_diff: Vec<f32> = wkv_out_flat
        .iter()
        .zip(expected_wkv_out.iter())
        .map(|(a, b)| (a - b).abs())
        .collect();
    ...
    println!(
        "WKV Output: max_diff={:.6e}, mean_diff={:.6e}",
        out_max, out_mean
    );
    println!(
        "WKV State:  max_diff={:.6e}, mean_diff={:.6e}",
        state_max, state_mean
    );
    println!(
        "Gap factors: output={:.0}x, state={:.0}x",
        out_max / 1e-3,
        state_max / 1e-6
    );
}
...
fn test_generation_topk_accuracy() {
    use std::path::Path;
    use web_rwkv::hip::{softmax_one_cpu, HipRuntime, Rwkv7Hip};
    ...
    let prompt_tokens: Vec<u32> = reference
        .i32("prompt_tokens")
        .iter()
        .map(|&x| x as u32)
        .collect();
    let ref_top_k_tokens = reference.i32("top_k_tokens");
    let ref_argmax_tokens: Vec<u32> = reference
        .i32("argmax_tokens")
        .iter()
        .map(|&x| x as u32)
        .collect();
    ...
    for i in 0..num_tokens {
        ...
        if i < 10 || i % 20 == 0 {
            let match_str = if hip_argmax == ref_argmax {
                "MATCH"
            } else {
                "MISS"
            };
            println!(
                "Step {:3}: HIP={:5} Ref={:5} [{}] ref_topk={:?}",
                i,
                hip_argmax,
                ref_argmax,
                match_str,
                &ref_topk[..3.min(ref_topk.len())]
            );
        }
    }
    ...
    println!(
        "Argmax matches: {}/{} ({:.1}%)",
        argmax_matches, num_tokens, argmax_rate
    );
    println!(
        "Top-k matches:  {}/{} ({:.1}%)",
        topk_matches, num_tokens, topk_rate
    );
    println!(
        "Generated: {:?}",
        &generated_tokens[..20.min(generated_tokens.len())]
    );
...
fn test_generation_topk_teacher_forcing() {
    ...
    for i in 0..num_tokens {
        ...
        if i < 10 || i % 20 == 0 {
            ...
            let ref_rank = indices
                .iter()
                .position(|&x| x as u32 == ref_argmax)
                .unwrap_or(vocab_size);
            println!(
                "Step {:3}: HIP={:5} Ref={:5} [{}] ref_rank={:4} hip_topk={:?}",
                i,
                hip_argmax,
                ref_argmax,
                match_str,
                ref_rank,
                &hip_topk[..3.min(hip_topk.len())]
            );
        }
    }
    ...
    println!(
        "HIP argmax in ref top-k:  {}/{} ({:.1}%)",
        topk_matches, num_tokens, topk_rate
    );
    println!(
        "Ref argmax in HIP top-10: {}/{} ({:.1}%)",
        in_top10_matches, num_tokens, in_top10_rate
    );
...
fn test_hip_step1_logits_debug() {
    ...
    let (argmax0, max_val0) = logits_510_data.iter().enumerate().fold(
        (0, f32::NEG_INFINITY),
    ...
    let (argmax1, max_val1) = logits_11_data.iter().enumerate().fold(
        (0, f32::NEG_INFINITY),
        |(max_idx, max_val), (idx, &val)| {
            if val > max_val {
                (idx, val)
    ...
    println!(
        "  Logit[51]: {:.4} (HIP argmax in autoregressive)",
        logits_11_data[51]
    );
    ...
    if Path::new("/tmp/python_step1_logits.npy").exists() {
        ...
        println!(
            "  HIP result:       argmax={}, token 47 at rank {}",
            argmax1, rank_47
        );
    }

    // Check logit statistics
    let logit_max = logits_11_data
        .iter()
        .cloned()
        .fold(f32::NEG_INFINITY, f32::max);
...
/// Test forward with a single batch (length equals padded length).
/// Verifies that forward produces consistent results for different state init patterns.
#[test]
#[cfg(feature = "hip")]
fn test_forward_masked_single_batch() {
    ...
    let max_state_diff = state1.att_states[0]
        .as_slice()
        .iter()
    ...
    assert!(max_state_diff < 1e-6, "States should match exactly");

    // Logits should match exactly
    let max_logit_diff = logits1
        .iter()
        .zip(logits2.iter())
        .map(|(&a, &b)| (a - b).abs())
        .fold(0.0f32, f32::max);
    ...
    assert!(max_logit_diff < 1e-6, "Logits should match exactly");
...
fn test_forward_chunking_consistency() {
    ...
    // Logits should match
    let max_logit_diff = logits_large
        .iter()
    ...
    // States should match
    let max_att_state_diff = state_large
        .att_states
        .iter()
    ...
    println!("  Tokens: {:?}", tokens);
    println!(
        "  Max logit diff (large vs small chunk): {:.6e}",
        max_logit_diff
    );
    ...
    if max_logit_diff >= 1e-4 {
        eprintln!(
            "Warning: logits differ across chunk sizes (max_diff={:.6e})",
            max_logit_diff
        );
    }
    if max_att_state_diff >= 1e-4 {
        eprintln!(
            "Warning: att state differs across chunk sizes (max_diff={:.6e})",
            max_att_state_diff
        );
    }
...
fn test_forward_masked_variable_batch() {
    ...
    if max_diff_batch0 >= 1e-5 {
        eprintln!(
            "Warning: batch 0 state differs (max_diff={:.6e})",
            max_diff_batch0
        );
    }
    if max_diff_batch1 >= 1e-5 {
        eprintln!(
            "Warning: batch 1 state differs (max_diff={:.6e})",
            max_diff_batch1
        );
    }
